...
from timm.data.readers.reader_image_folder import load_class_map
import torch
...
group.add_argument('--mapping', default=None, type=str,
                   help='Mapping for input and target keys.')
...
group.add_argument('--name', default='', type=str, metavar='NAME',
                     help='train name, for logging')
...
def validate(
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            with amp_autocast():
                ...
                loss = loss_fn(output, target)
            gt += target.cpu().numpy().astype('i').tolist()
            pred += output.cpu().numpy().argmax(1).astype('i').tolist()
    ...
    classwise_prec = precision_score(gt, pred, average=None)
    classwise_recall = recall_score(gt, pred, average=None)
    classwise_f1 = f1_score(gt, pred, average=None)
    precision_dict = {f'precision/{k}': v for k, v in zip(mapping, classwise_prec)}
    recall_dict = {f'recall/{k}': v for k, v in zip(mapping, classwise_recall)}
    metrics = OrderedDict([('loss', losses_m.avg), ('top1', top1_m.avg), ('top5', top5_m.avg),
                            ('precision/macro', classwise_prec.mean()), ('recall/macro', classwise_recall.mean()),
                            ('f1/macro', classwise_f1.mean()), *precision_dict.items(), *recall_dict.items()])
